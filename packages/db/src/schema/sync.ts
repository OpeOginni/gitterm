...
export const syncProject = pgTable(
  "sync_project",
  {
...
export const syncManifest = pgTable(
  "sync_manifest",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    userId: text("user_id")
      .references(() => user.id)
      .notNull(),
    syncProjectId: uuid("sync_project_id")
      .references(() => syncProject.id)
      .notNull(),
    ...
    files: jsonb("files")
      .notNull()
      .$type<Record<string, { size: number; chunks: string[]; mode?: number }>>(),
  },
  (table) => [
    uniqueIndex("sync_manifest_user_id_sync_project_id_unique").on(
      table.userId,
      table.syncProjectId,
    ),
  ],
