...
ALTER TABLE "region" DROP CONSTRAINT "region_external_region_identifier_unique";--> statement-breakpoint
ALTER TABLE "cloud_provider" ADD COLUMN "provider_key" text DEFAULT 'local' NOT NULL;--> statement-breakpoint
...
CREATE UNIQUE INDEX "provider_agent_image_unique" ON "provider_agent_image" USING btree ("cloud_provider_id","agent_type_id",coalesce("workspace_profile", ''));--> statement-breakpoint
CREATE UNIQUE INDEX "region_provider_external_identifier_unique" ON "region" USING btree ("cloud_provider_id","external_region_identifier");