...
export const tunnelRouter = router({
	mintToken: protectedProcedure
		...
		.mutation(async ({ input, ctx }) => {
			...
			const [ws] = await db.select().from(workspace).where(eq(workspace.id, input.workspaceId)).limit(1);
	...
	mintTokenWithAgentToken: publicProcedure
		...
		.mutation(async ({ input }) => {
			...
			try {
				...
				throw new TRPCError({ code: "UNAUTHORIZED", message: error instanceof Error ? error.message : "Invalid token" });
			}
