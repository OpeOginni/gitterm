...
const updateUserSchema = z.object({
  id: z.string().min(1),
  plan: z.enum(["free", "tunnel", "pro", "enterprise"]).optional(),
...
const createUserSchema = z.object({
  ...
  plan: z.enum(["free", "tunnel", "pro", "enterprise"]).default("free"),
});
...
export const usersRouter = router({
  ...
  list: adminProcedure
    ...
    .query(async ({ input }) => {
      ...
      const sortColumn = sortBy === "email" ? user.email : sortBy === "name" ? user.name : user.createdAt;
  ...
  create: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      if (isGitHubAuthEnabled()) {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "User creation is not available when GitHub authentication is enabled. Users should sign up via GitHub.",
        });
      }
  ...
  get: adminProcedure
    ...
    .query(async ({ input }) => {
      ...
      const workspaces = await db
        .select()
        .from(workspace)
        .where(eq(workspace.userId, input.id));
