...
echo "[git-push] Credential helper configured: $([ -x "$(git config credential.helper 2>/dev/null || true)" ] && echo 'YES' || echo 'NO')"
...
# Check if GitHub credential helper is available
if ! [ -x "$(git config credential.helper 2>/dev/null || true)" ]; then
