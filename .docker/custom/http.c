...
int callback_http(struct lws *wsi, enum lws_callback_reasons reason, void *user, void *in, size_t len) {
  ...
  switch (reason) {
    ...
    case LWS_CALLBACK_HTTP_BODY:
      ...
      lwsl_notice("[HTTP_BODY] Callback fired for path: %s, len: %zu\n", pss->path, len);
      
      if (!pss->post_body) {
        // First chunk - allocate buffer
        pss->post_body_max = 4096; // Max 4KB for POST body
        pss->post_body = malloc(pss->post_body_max);
        if (!pss->post_body) {
          lwsl_err("Failed to allocate POST body buffer\n");
          return -1;
        }
        pss->post_body_len = 0;
        lwsl_notice("[HTTP_BODY] Allocated buffer for POST body\n");
      }
      
      // Append this chunk to the buffer
      if (pss->post_body_len + len <= pss->post_body_max) {
        memcpy(pss->post_body + pss->post_body_len, in, len);
        pss->post_body_len += len;
        lwsl_notice("[HTTP_BODY] Accumulated %zu bytes, total: %zu\n", len, pss->post_body_len);
        
        // Debug: print the body content
        char debug_body[256];
        size_t debug_len = pss->post_body_len < sizeof(debug_body) - 1 ? pss->post_body_len : sizeof(debug_body) - 1;
        memcpy(debug_body, pss->post_body, debug_len);
        debug_body[debug_len] = '\0';
        lwsl_notice("[HTTP_BODY] Body content: %s\n", debug_body);
      } else {
        lwsl_warn("POST body exceeds maximum size\n");
      }
      break;
