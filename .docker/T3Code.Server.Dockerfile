...
# T3 drives the OpenCode 1.x CLI itself; this is unrelated to the OpenCode 2 workspace images.
ARG T3_OPENCODE_VERSION=1
...
RUN echo "t3 install cache bust: ${T3_INSTALL_CACHE_BUST}" \
    ...
    && npm install -g "opencode-ai@${T3_OPENCODE_VERSION}" --no-audit --fund=false \
